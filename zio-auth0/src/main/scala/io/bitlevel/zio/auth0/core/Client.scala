...
case class Client(configuration: Configuration, semaphore: Semaphore, ref: Ref[TokenHolder], auth: AuthAPI, management: ManagementAPI) {
   ...
   */
  private[auth0] def execute[A](request: () => Request[A]): Task[A] = {
